...
export default function KanbanTaskCard({ task, isDragging }) {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <h3 className="text-sm font-bold text-[#1D3557] dark:text-slate-100 leading-snug line-clamp-2">
          {task.title}
        </h3>
        <span className={`shrink-0 px-2 py-0.5 text-[10px] font-bold rounded-full ${priorityClass}`}>
          {task.priority}
        </span>
      </div>
