...
export default function TodaysFocus() {
  ...
  return (
    <div className="w-full h-[23em] flex flex-col rounded-2xl border border-[#1f4d63] bg-[#e8f4ff] shadow-md hover:shadow-lg transition-all duration-200">
      {/* Header */}
      <div className="border-b border-[#1f4d63] px-4 py-3 flex items-center gap-2">
        <TodayRoundedIcon sx={{ color: "#1f4d63", fontSize: "1.2rem" }} />
      ...
      {!loading && todaysTasks.length > 0 && (
        <div className="px-4 pt-3 pb-1">
          <div className="flex justify-between text-xs text-[#1f4d63] mb-1 font-medium">
            <span>{completedCount} of {todaysTasks.length} done</span>
            <span>{Math.round((completedCount / todaysTasks.length) * 100)}%</span>
          </div>
          <div className="w-full h-1.5 rounded-full bg-[#c2dff0]">
            <div
              className="h-1.5 rounded-full bg-[#1f4d63] transition-all duration-500"
        ...
        {!loading && todaysTasks.length === 0 && (
          <div className="flex flex-col items-center py-5 text-center">
            <SentimentSatisfiedRoundedIcon
              sx={{ color: "#34d399", fontSize: "2.2rem" }}
            />
            ...
            >
              {task.status === "Completed" ? (
                ...
                <RadioButtonUncheckedRoundedIcon
                  sx={{ color: "#9ca3af", fontSize: "1.1rem", mt: "2px", flexShrink: 0 }}
                />
              )}
              ...
              <div className="flex-1 min-w-0">
                ...
                <span
                  className={`mt-1 inline-block text-xs px-2 py-0.5 rounded-full font-medium ${priorityStyle(task.priority)}`}
                >
                  {task.priority}
                </span>
              </div>
